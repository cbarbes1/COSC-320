...
private:
    ...
    void addElementRec(RBTreeNode<T>*);
...
template<class T>
void set<T>::addElementRec(RBTreeNode<T> *nodePtr)
{
    if(nodePtr != RBTree<T>::NIL){
        addElementRec(nodePtr->left);
        insert(nodePtr->value);
        addElementRec(nodePtr->right);
    }
}

template<class T>
void set<T>::deleteElementRec(RBTreeNode<T> *nodePtr)
{
    if(nodePtr != RBTree<T>::NIL){
        addElementRec(nodePtr->left);
        erase(nodePtr->value);
...
set<T>* set<T>::operator+(set<T>& right)
{
    ...
    newSet->addElementRec(RBTree<T>::root);
    newSet->addElementRec(right.RBTree<T>::root);
    return newSet;
}
...
set<T>* set<T>::operator-(set<T>& right)
{
    set<T> *newSet = new set<T>;
    newSet->addElementRec(RBTree<T>::root);
    newSet->deleteElementRec(right.RBTree<T>::root);
    return newSet;
}
...
{
    set<T> *newSet = new set<T>;
    newSet = (this + right) - ((this - right) + (right - this));
    return newSet;
}
...
{
    os<<"{ ";
    obj.InOrderDisplay(obj.RBTree<T>::root, os);
    os<<" }\n";
    return os;
}
