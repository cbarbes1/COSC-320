...
protected:
    ...
    // number of nodes
	int numNodes(RBTreeNode<T>*) const;
...
template<class T>
int RBTree<T>::IPL(RBTreeNode<T> *t, int h) {
    if (!t)
        return 0;

    return IPL(t->left, h + 1) + IPL(t->right, h + 1) + h;
}
...
template<class T>
int RBTree<T>::numNodes(RBTreeNode<T>*nodePtr) const{
	if(nodePtr){
